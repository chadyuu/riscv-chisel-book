...
class Core extends Module {
  ...
  alu_out := MuxCase(0.U(WORD_LEN.W), Seq(
    ...
    (exe_fun === ALU_PCNT)  -> PopCount(op1_data)
  ))
  ...
  val csr_wdata = MuxCase(op1_data, Seq( // デフォルト値はWB_CSR_W用
  ...
  //**********************************
  // IO & Debug
  io.exit := (inst === 0xc0001073L.U(WORD_LEN.W)) //0xc0001073.U causes error over the UInt coverage.
